...
class PreviewFile:
    ...
    def __init__(self, file_item, record_pid_value, url=None):
        ...
        self.uri = url or url_for(
            "{{cookiecutter.package_name}}_records.record_file_download",
...
def record_detail(record=None, files=None, pid_value=None, is_preview=False):
    ...
    return render_template(
        "{{cookiecutter.package_name}}/records/detail.html",
        record=record_ui,
        pid=pid_value,
        files=files_dict,
...
def record_export(
    ...
    exporter = current_app.config.get("APP_RECORD_EXPORTERS", {}).get(
        export_format
    )
    if exporter is None:
        abort(404)
...
def record_tombstone_error(error):
    """Tombstone page."""
    return render_template("{{cookiecutter.package_name}}/records/tombstone.html"), 410
