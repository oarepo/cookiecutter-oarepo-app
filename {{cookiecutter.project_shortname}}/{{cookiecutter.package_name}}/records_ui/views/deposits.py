...
def get_search_url():
    ...
    return current_app.config["APP_ROUTES"]["record_search"]
...
def new_record():
    ...
    if "doi" in current_rdm_records.records_service.config.pids_providers:
        ...
        record["pids"] = {}
    defaults = current_app.config.get("APP_DEPOSIT_FORM_DEFAULTS") or {}
...
def dashboard(dashboard_name=None):
    ...
    return render_template(
        "{{cookiecutter.package_name}}/records/dashboard.html",
...
def deposit_create(community=None):
    ...
    return render_template(
        "{{cookiecutter.package_name}}/records/deposit.html",
